...
{
  ui->setupUi (this);
  udp_reciver = new UdpReciver();
  connect(udp_reciver, SIGNAL(signal_new_info(QByteArray)),
          this, SLOT(slot_update_info(QByteArray)));
...
void MainWindow::portOpenedSuccess()
{
    ...
    updateTimer.start (1);                                                                // Slot is refreshed 20 times per second
...
void MainWindow::slot_update_info(QByteArray bytearray){
                                                   // If any bytes are available
        QByteArray data = bytearray;                                          // Read all data in QByteArray
